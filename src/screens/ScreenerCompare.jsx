...
import { Th, Td, ScorePip, PageHead, CompositeMini } from "../components/shared.jsx";
...
const FILTER_DEFS = [
  { key: "capMin", label: "Market cap ≥", min: 0, max: 3500, step: 50, unit: "$B", get: (c) => c.marketCap, cmp: "gte" },
  { key: "peMax", label: "P/E ≤", min: 5, max: 120, step: 1, unit: "×", get: (c) => c.pe, cmp: "lte" },
  { key: "pegMax", label: "PEG ≤", min: 0.5, max: 5, step: 0.1, unit: "×", get: (c) => c.peg, cmp: "lte" },
  { key: "pbMax", label: "P/B ≤", min: 1, max: 50, step: 1, unit: "×", get: (c) => c.pb, cmp: "lte" },
  { key: "divMin", label: "Div yield ≥", min: 0, max: 5, step: 0.1, unit: "%", get: (c) => c.divYield, cmp: "gte" },
  { key: "roeMin", label: "ROE ≥", min: 0, max: 100, step: 5, unit: "%", get: (c) => c.roe, cmp: "gte" },
  { key: "growthRevMin", label: "Rev growth ≥", min: -10, max: 100, step: 5, unit: "%", get: (c) => c.revGrowth, cmp: "gte" },
  { key: "valueMin", label: "Value score ≥", min: 0, max: 100, step: 5, unit: "", get: (c) => c.scores.value, cmp: "gte" },
  { key: "growthMin", label: "Growth score ≥", min: 0, max: 100, step: 5, unit: "", get: (c) => c.scores.growth, cmp: "gte" },
  { key: "healthMin", label: "Health score ≥", min: 0, max: 100, step: 5, unit: "", get: (c) => c.scores.health, cmp: "gte" },
  { key: "momentumMin", label: "Momentum ≥", min: 0, max: 100, step: 5, unit: "", get: (c) => c.scores.momentum, cmp: "gte" },
];
...
function Screener({ go, watch, toggleWatch }) {
  ...
  const applyPreset = (name) => {
    ...
    FILTER_DEFS.forEach((d) => {
      if (p[d.key] != null) f[d.key] = p[d.key];
    });
    setFilters(f);
  };
  ...
  const matchPass = (c) => FILTER_DEFS.every((d) => {
    const v = filters[d.key];
    if (v == null) return true;
    const cv = d.get(c);
    if (cv == null) return false;
    return d.cmp === "lte" ? cv <= v : cv >= v;
  });

  let results = DATA.companies.filter(matchPass);
  results = [...results].sort((a, b) => {
    const get = (c) => ({ ticker: c.ticker, price: c.price, pe: c.pe ?? 1e9, peg: c.peg ?? 1e9,
      div: c.divYield, roe: c.roe, rev: c.revGrowth, value: c.scores.value, growth: c.scores.growth, composite: c.scores.composite }[sort.k]);
    ...
    <div className="fade-up" style={{ display: "flex", height: "100%", minHeight: 0 }}>
      ...
      <div style={{ flex: 1, minWidth: 0, overflowY: "auto", padding: 24 }}>
        <PageHead title="Screener"
          sub={<span>{results.length} of {DATA.companies.length} companies match{activeFilters.length ? ` · ${activeFilters.length} active filter${activeFilters.length > 1 ? "s" : ""}` : " · no filters"}</span>}
...
function Compare({ go, set, setSet }) {
  ...
  const cos = set.map((t) => DATA.byTicker[t]).filter(Boolean);
  const available = DATA.companies.filter((c) => !set.includes(c.ticker));
  const remove = (t) => setSet((s) => s.filter((x) => x !== t));
  const add = (t) => { setSet((s) => [...s, t].slice(0, 5)); setAdding(false); };
    ...
    <div className="fade-up" style={{ padding: 24, height: "100%", overflow: "auto" }}>
      ...
      <Card pad={0} style={{ minWidth: "fit-content" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: grid, position: "sticky", top: 0, background: "var(--surface)", zIndex: 2, borderBottom: "1px solid var(--border)" }}>
          ...
          {cos.length < 5 && (
            <div style={{ padding: 16, borderLeft: "1px solid var(--border)", display: "grid", placeItems: "center", position: "relative" }}>
              ...
              {adding && (
                <div style={{ position: "absolute", top: 70, left: 8, right: 8, background: "var(--surface)", border: "1px solid var(--border)", borderRadius: 8, boxShadow: "var(--shadow)", padding: 5, zIndex: 10, maxHeight: 240, overflowY: "auto" }}>
                  {available.map((c) => (
                    <div key={c.ticker} onClick={() => add(c.ticker)} style={{ display: "flex", alignItems: "center", gap: 8, padding: "6px 8px", borderRadius: 6, cursor: "pointer" }}
        ...
        {["value", "growth", "health", "momentum", "composite"].map((sk) => {
          ...
          cos.forEach((c, i) => { if (c.scores[sk] > bestV) { bestV = c.scores[sk]; best = i; } });
          return (
